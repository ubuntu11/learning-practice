...
def remove_event_in_schedule():
  ...
  for eventID in eventIDList:
    db.schedule.update_one({"_id":ObjectId(eventID)},{"$set":{'show':0}})
  status = "完成"
  return jsonify(status=status)
#------------------------------------------------------------------------------------------------------
# 提出中止待命排程事件
...
def soe_get():
  ...
  total = len(list(db.SOE.find({**time_condiction,**soeType_condiction}, {'_id': 1})))
...
def report_get():
  ...
  total = len(list(db.report.find({**time_condiction,**reportType_condiction}, {'_id': 1})))
