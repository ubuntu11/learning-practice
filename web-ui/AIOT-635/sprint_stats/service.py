    ...
    for issue in issue_unsorted:
        for work_log in issue.work_logs:
            ...
            if work_log.started_at < sprint.start or work_log.started_at > sprint.end:
                continue
            if str(work_log.started_at.date()) in time_sheet_by_date:
                time_sheet_by_date[str(work_log.started_at.date())] += work_log.time_spent_seconds
            else:
                time_sheet_by_date[str(work_log.started_at.date())] = work_log.time_spent_seconds

    return sorted(issue_unsorted, key=attrgetter('status', 'id')), time_sheet_by_date
